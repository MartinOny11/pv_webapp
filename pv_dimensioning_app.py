...
class House:
    ...
    thermal_demand: np.ndarray = None  # Hourly heat demand for the year (kWh_th per hour)
...
class HouseholdModel:
    ...
    def _load_climate_data(self):
        ...
        try:
            ...
            while col_idx < df.shape[1] and locations_loaded < 10:
                try:
                    ...
                    if len(temp_data) >= 8760 and len(rad_data) >= 8760:
                        ...
                        location = Location(
                            ...
                            global_radiation=np.maximum(rad_data[:8760], 0)
                        )
            ...
            self.climate_data['Prague'] = Location(
                ...
                latitude=50.0,
                longitude=14.5,
                temperature=10 + 15 * np.sin(np.arange(8760) * 2 * np.pi / 8760 - np.pi/2),
                global_radiation=np.maximum(
                    500 * np.sin(np.arange(8760) * 2 * np.pi / 8760) * np.sin(np.arange(8760) * np.pi / 24),
                    0
                )
            )

    def _load_house_profiles(self):
        """Load house thermal demand profiles from P4 (P4 is in Wh/h -> convert to kWh/h)"""
        try:
            df = pd.read_excel(
            ...
            for col_idx in range(1, min(51, df.shape[1])):  # 50 house profiles
                thermal_data_wh = pd.to_numeric(df.iloc[5:8765, col_idx], errors='coerce').fillna(0).values

                if len(thermal_data_wh) >= 8760:
                    # ✅ P4 is Wh per hour -> convert to kWh per hour
                    thermal_kwh = (thermal_data_wh[:8760].astype(float)) / 1000.0
    ...
    def assign_house_profile_id(self, floor_area: float, floors: int, year: int) -> int:
        a = self._bin_floor_area(floor_area)
        y = self._bin_year(year)
        f = self._bin_floors(floors)
    ...
    def generate_consumption_profile(self,
        ...
        if wash_days_count > 0:
            ...
            for day in washing_days:
                ...
                if wash_hour < 8760:
                    consumption[wash_hour:wash_hour+2] += 1.5 * f["peak_mult"]
    ...
    def add_ev_consumption(self, consumption: np.ndarray,
        ...
        for i in range(charging_days):
            ...
            if charge_start_hour < 8760:
                charge_hours = int(np.ceil(charge_per_session / 7.0))
    ...
    def calculate_pv_production(self,
        ...
        for hour in range(8760):
            ...
            temp_coefficient = 1.0 - 0.004 * (temp - 25)
    ...
    def calculate_energy_balance(self,
        ...
        for hour in range(8760):
            ...
            if battery_capacity > 0:
                if surplus > 0:
                    charge = min(surplus, battery_capacity - battery_state, battery_capacity * 0.5)
                    battery_state += charge * 0.95
                    battery_charge[hour] = charge
                    surplus -= charge

                if deficit > 0:
                    discharge = min(deficit, battery_state, battery_capacity * 0.5)
                    battery_state -= discharge
                    battery_discharge[hour] = discharge * 0.95
    ...
    def calculate_economics(self,
                           ...
                           feed_in_tariff: float = 1.5,
                           pv_cost_per_kwp: float = 22000,
                           battery_cost_per_kwh: float = 9000,
                           subsidy: float = 100000,
                           lifetime: int = 25,
                           discount_rate: float = 0.05) -> Dict:
        pv_cost = installed_power * pv_cost_per_kwp
    ...
    def optimize_pv_size(self,
        ...
        for pv_power in np.arange(min_power, max_power + step, step):
            for battery_cap in [0, pv_power * 0.5, pv_power]:
                ...
                if economics['npv'] > best_npv and economics['irr'] >= 5:
...
def main():
    ...
    model.add_user(gender='female', age=44, status='classic_8_16')
    model.add_user(gender='male', age=46, status='morning_shift')
    model.add_user(gender='male', age=17, status='student')
